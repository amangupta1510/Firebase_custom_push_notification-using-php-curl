...
Window.type = "{{$user->notification_type}}";
$('#id').val('{{$user->id}}');
$('#title').val('{{$user->title}}');
$('#body').val('{{$user->body}}');
$('#title_long').val('{{$user->title_long}}');
$('#body_long').val('{{$user->body_long}}');
$('#title_line').val('{{$user->title_line}}');
$('#body_line1').val('{{$user->body_line1}}');
$('#body_line2').val('{{$user->body_line2}}');
$('#body_line3').val('{{$user->body_line3}}');
$('#body_line4').val('{{$user->body_line4}}');
$('#body_line5').val('{{$user->body_line5}}');
$('#body_line6').val('{{$user->body_line6}}');
$('#body_line7').val('{{$user->body_line7}}');
$('#body_line8').val('{{$user->body_line8}}');
$('#body_line9').val('{{$user->body_line9}}');
$('#body_line10').val('{{$user->body_line10}}');
$('#icon').val('{{$user->icon}}');
$('#image').val('{{$user->image}}');
$('#summary').val('{{$user->summary}}');

$('.edit_title').text('{{$user->title}}');
$('.edit_body').text('{{$user->body}}');
$('.edit_title_long').text('{{$user->title_long}}');
$('.edit_body_long').text('{{$user->body_long}}');
@if($user - > title_line) $('.edit_title_line').text('{{$user->title_line}}');
@endif
@if($user - > body_line1) $('.edit_body_line1').text('{{$user->body_line1}}');
@endif
@if($user - > body_line2) $('.edit_body_line2').text('{{$user->body_line2}}');
@endif
@if($user - > body_line3) $('.edit_body_line3').text('{{$user->body_line3}}');
@endif
@if($user - > body_line4) $('.edit_body_line4').text('{{$user->body_line4}}');
@endif
@if($user - > body_line5) $('.edit_body_line5').text('{{$user->body_line5}}');
@endif
@if($user - > body_line6) $('.edit_body_line6').text('{{$user->body_line6}}');
@endif
@if($user - > body_line7) $('.edit_body_line7').text('{{$user->body_line7}}');
@endif
@if($user - > body_line8) $('.edit_body_line8').text('{{$user->body_line8}}');
@endif
@if($user - > body_line9) $('.edit_body_line9').text('{{$user->body_line9}}');
@endif
@if($user - > body_line10) $('.edit_body_line10').text('{{$user->body_line10}}');
@endif
@if($user - > icon != null || $user - > icon != '') $('.edit_icon').attr('src', '{{$user->icon}}');
@endif
@if($user - > image != null || $user - > image != '') $('.edit_image').attr('src', '{{$user->image}}');
@endif
$('.edit_summary').text('{{$user->summary}}');
@endforeach
...
$("#icon").keyup(function() {
    ...
    imageExists($('#icon').val(), function(exists) {
        if (exists) { Window.icon = 'true';
            $('.edit_icon').attr('src', $('#icon').val()); } else { Window.icon = 'false';
...
$("#image").keyup(function() {
    ...
    imageExists($('#image').val(), function(exists) {
        if (exists) { Window.image = 'true';
            $('.edit_image').attr('src', $('#image').val()); } else { Window.image = 'false';
...
function save() {
    ...
    if (count == 0) {
        ...
        $.ajax({
            type: 'POST',
            url: '{{ route('
            admin - edit_notification_submit ') }}',
            data: {
